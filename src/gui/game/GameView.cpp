...
#include "graphics/RayTraceRenderer.h"
...
void GameView::NotifyRendererChanged(GameModel * sender)
{
	...
	raytrace = sender->GetRayTraceRenderer();
...
void GameView::OnKeyPress(int key, int scan, bool repeat, bool shift, bool ctrl, bool alt)
{
	...
	case SDL_SCANCODE_R:
		if (ctrl)
		{
			rendererSettings->rayTraceEnabled = !rendererSettings->rayTraceEnabled;
			if (raytrace)
			{
				raytrace->SetEnabled(rendererSettings->rayTraceEnabled);
			}
			if (rendererSettings->rayTraceEnabled)
			{
				buttonTip = String("\x0F\xEF\xEF\020Ray trace lighting: ON");
			}
			else
			{
				buttonTip = String("\x0F\xEF\xEF\020Ray trace lighting: OFF");
			}
			buttonTipShow = 120;
		}
		break;
...
void GameView::OnDraw()
{
	...
	// Ray traced lighting with async PBO readback
	if (raytrace && rendererSettings->rayTraceEnabled && raytrace->IsEnabled())
	{
		raytrace->SetResolution(
			rendererSettings->rayTraceResolution,
			rendererSettings->rayTraceResolution * YRES / XRES
		);
