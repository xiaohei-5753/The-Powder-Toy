	...
	view(newView)
{
	...
	ren = new Renderer();
	raytrace = new RayTraceRenderer();
	if (raytrace->Init())
	{
		fprintf(stdout, "RayTraceRenderer initialized successfully\n");
	}
	else
	{
		fprintf(stdout, "RayTraceRenderer initialization failed (GL 4.3 not available?)\n");
	}
...
RayTraceRenderer * GameModel::GetRayTraceRenderer()
{
	return raytrace;
}
