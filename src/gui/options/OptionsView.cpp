...
OptionsView::OptionsView() : ui::Window(ui::Point(-1, -1), ui::Point(320, 340))
{
	...
	addSeparator();
	addLabel(0, "\bgGPU Ray traced lighting");
	addLabel(1, "Renders global illumination via OpenGL compute shader");
	rayTraceEnabled = addCheckbox(0, "Enable ray traced lighting", "Press Ctrl+R in-game to toggle", [this] {
		c->SetRayTraceEnabled(rayTraceEnabled->GetChecked());
	});
	addLabel(1, "Resolution (higher = better quality, slower)");
	rayTraceResolution = addTextboxWithPreview("Resolution (X)", false, [this](String value, bool defocus) {
		UpdateRayTraceResolution(value, defocus);
	}).first;
	addLabel(1, "Sampling radius (higher = better lighting, slower)");
...
void OptionsView::NotifySettingsChanged(OptionsModel * sender)
{
	...
	rayTraceEnabled->SetChecked(sender->GetRayTraceEnabled());
	if (!rayTraceResolution->IsFocused())
	{
		rayTraceResolution->SetText(String::Build((int)sender->GetRayTraceResolution()));
	}
	if (!rayTraceRadius->IsFocused())
	{
...
void OptionsView::UpdateRayTraceResolution(String value, bool defocus)
{
	String numericsOnly;
	for (auto ch : value)
	{
	...
	if (numericsOnly.empty())
	{
		if (defocus)
			rayTraceResolution->SetText(String::Build((int)XRES));
		return;
	}
	int res = numericsOnly.ToNumber<int>(true);
	res = std::clamp(res, 32, XRES);
	if (defocus)
	{
		rayTraceResolution->SetText(String::Build(res));
	}
	c->SetRayTraceResolution(res);
}

void OptionsView::UpdateRayTraceRadius(String value, bool defocus)
{
	String numericsOnly;
	for (auto ch : value)
	{
	...
	{
		if (defocus)
			rayTraceRadius->SetText(String::Build((int)64));
		return;
	}
	int rad = numericsOnly.ToNumber<int>(true);
	rad = std::clamp(rad, 1, 256);
	if (defocus)
	{
